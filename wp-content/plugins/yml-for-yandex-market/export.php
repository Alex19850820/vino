...
if (is_multisite()) { 
 ...
 if (isset($_REQUEST['yfym_submit_send_stat'])) {
	...
	$mail_content .= "Версия плагина: 2.0.1". PHP_EOL;;
	$mail_content .= "Версия WP: ".get_bloginfo('version'). PHP_EOL;
	if (!class_exists('YmlforYandexMarketPro')) {$mail_content .= "Pro: не активна". PHP_EOL;} else {$mail_content .= "Pro: активна". PHP_EOL;}
	if (!class_exists('YmlforYandexMarketPromosExport')) {$mail_content .= "Promos Export: не активна". PHP_EOL;} else {$mail_content .= "Promos Export: активна". PHP_EOL;}
 ...
 if (isset($_REQUEST['yfym_submit_reset'])) {
  if (!empty($_POST) && check_admin_referer('yfym_nonce_action','yfym_nonce_field')) { 
	...
	add_blog_option(get_current_blog_id(),'yfym_version', '2.0.1');
 ...
 if (isset($_REQUEST['yfym_submit_action'])) {
  if (!empty($_POST) && check_admin_referer('yfym_nonce_action','yfym_nonce_field')) {
	...
	update_blog_option(get_current_blog_id(), 'yfym_version', '2.0.1');
 ...
 if (isset($_REQUEST['yfym_submit_action'])) {
  if (!empty($_POST) && check_admin_referer('yfym_nonce_action','yfym_nonce_field')) {
	...
	update_option('yfym_version', '2.0.1');
	if (isset($_POST['yfym_skip_missing_products'])) {
 ...
 <div class="wrap">
  <h1><?php _e('Exporter Yandex Market', 'yfym'); ?></h1>
	...
	if (!class_exists('YmlforYandexMarketPromosExport')) {
		print '<div class="notice notice-info is-dismissible"><p><span style="color: green;">Promos Export</span> '. __('is an extension that allows you to increase sales by unloading Promotions (promos, gifts) on the Yandex Market', 'yfym'). '. <a href="'.ADMIN_COOKIE_PATH.'/admin.php?page=yfymextensions">'. __('Learn More', 'yfym'). '</a>.</p></div>';
	} else {
		print '<div class="notice notice-info is-dismissible"><p>'. __('Thank you for purchasing the Promos Export extension', 'yfym'). '! <a href="https://icopydoc.ru/kejs-dobavlyaem-informatsiyu-o-podarkah-na-yandeks-market/">'. __('Read usage case', 'yfym'). '</a>.</p></div>';
	}
	if (!class_exists('YmlforYandexMarketPro')) {
		...
		print '<div class="notice notice-success is-dismissible"><p>'. __('Thank you for purchasing the Yml for Yandex Market Pro extension', 'yfym'). '! <a href="https://icopydoc.ru/para-sovetov-po-ispolzovaniyu-yml-for-yandex-market-pro/">'. __('Read usage tips', 'yfym'). '</a>.</p></div>';		
	} ?>  
	 ...
	 <div class="postbox">
	   <div class="inside">
		...
		<table class="form-table"><tbody>
		 ...
		 <tr>
			<th scope="row"><label for="yfym_keeplogs"><?php _e('Keep logs', 'yfym'); ?></label></th>
			<td class="overalldesc">
		...
		<table class="form-table"><tbody>
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($yfym_barcode, 'none'); ?>><?php _e( 'None', 'yfym'); ?></option>
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($yfym_vendorcode, 'none'); ?>><?php _e('None', 'yfym'); ?></option>
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($expiry, 'none'); ?>><?php _e( 'None', 'yfym'); ?></option>
				<?php foreach (get_attributes() as $attribute) : ?>	
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($downloadable, 'none'); ?>><?php _e( 'None', 'yfym'); ?></option>
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($age, 'none'); ?>><?php _e( 'None', 'yfym'); ?></option>
				<?php foreach (get_attributes() as $attribute) : ?>	
		 ...
		 <tr>
			...
			<td class="overalldesc">
			 <select name="yfym_model" id="yfym_model">		 
				<option value="off" <?php selected($model, 'none' ); ?>><?php _e( 'None', 'yfym'); ?></option>
				<?php foreach (get_attributes() as $attribute) : ?>	
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($vendor, 'none' ); ?>><?php _e('None', 'yfym'); ?></option>
				<?php foreach (get_attributes() as $attribute) : ?>	
		 ...
		 <tr>
			...
			<td class="overalldesc">
				...
				<option value="off" <?php selected($country_of_origin, 'none'); ?>><?php _e( 'None', 'yfym'); ?></option>
				<?php foreach (get_attributes() as $attribute) : ?>	
		...
		<?php do_action('yfym_after_manufacturer_warranty'); ?>	
		 <tr>
			...
			<td class="overalldesc">			
				...
				<option value="off" <?php selected($sales_notes_cat, 'none'); ?>><?php _e('Disable use of Sales notes', 'yfym'); ?></option>
				<?php foreach (get_attributes() as $attribute) : ?>	
	...
	<div id="postbox-container-2" class="postbox-container"><div class="meta-box-sortables">
	 ...
	 <div class="postbox">
	  ...
	  <ul>
		...
		<li>- <?php _e('Support the project financially. Even $2 is a help!', 'yfym'); ?><a href="https://icopydoc.ru/donate/" target="_blank"> <?php _e('Donate now', 'yfym'); ?></a>.</li>
