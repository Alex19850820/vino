...
function yfym_feed_header() {
 $result_yml = '';
 $unixtime = current_time('Y-m-d H:i'); // время в unix формате 
 update_option('yfym_date_sborki', $unixtime);		
 $shop_name = yfym_optionGET('yfym_shop_name');
 $company_name = yfym_optionGET('yfym_company_name');
 ...
 $yfym_delivery_options = yfym_optionGET('yfym_delivery_options');
 if ($yfym_delivery_options == 'on') {
	$delivery_cost = yfym_optionGET('yfym_delivery_cost');
	$delivery_days = yfym_optionGET('yfym_delivery_days');
	$order_before = yfym_optionGET('yfym_order_before');
 ...
 // магазин 18+
 $adult = yfym_optionGET('yfym_adult');
...
function yfym_unit($postId) {
 yfym_error_log('Стартовала yfym_unit. Файл: functions.php; Строка: '.__LINE__, 0);	
			
 $cur_post = get_post($postId);
 if ($cur_post == null)	{return false;}
 $result_yml = ''; 
 $product = wc_get_product($postId);

 // что выгружать
 if ($product->is_type('variable')) {
	$yfym_whot_export = yfym_optionGET('yfym_whot_export');
	if ($yfym_whot_export == 'simple') {return $result_yml;}
 }
 ...
 $store = yfym_optionGET('yfym_store');
 $result_yml_store = "<store>$store</store>".PHP_EOL;
		  
 $pickup = yfym_optionGET('yfym_pickup');
 $delivery = yfym_optionGET('yfym_delivery');		  
 ...
 */	
 ...
 $result_yml_name = $cur_post->post_title; // название товара
 $result_yml_name = apply_filters('yfym_change_name', $result_yml_name, get_the_id());
		  
 // описание
 $yfym_desc = yfym_optionGET('yfym_desc');
 $result_yml_desc = '';
 if ($yfym_desc == 'full') {
	$description_yml = $cur_post->post_content;			
 } else {
	$description_yml = $cur_post->post_excerpt;
 }		 
 if (!empty($description_yml)) {
	...
	$description_yml = apply_filters('yfym_description_filter', $description_yml, $postId);	  			
 ...
 */			  
 ...
 if ($product->is_type('variable')) {
	yfym_error_log('У нас вариативный товар. Файл: functions.php; Строка: '.__LINE__, 0);	
	...
	for ($i = 0; $i<$variation_count; $i++) {
		...
		*/
		...
		if (class_exists('YmlforYandexMarketPro')) {
			if ((yfym_optionGET('yfymp_compare_value') !== false) && (yfym_optionGET('yfymp_compare_value') !== '')) {
			 $yfymp_compare_value = yfym_optionGET('yfymp_compare_value');
			 $yfymp_compare = yfym_optionGET('yfymp_compare');			 
			 if ($yfymp_compare == '>=') {
		...
		$yfym_skip_missing_products = yfym_optionGET('yfym_skip_missing_products');
		if ($yfym_skip_missing_products == 'on') {
			if ($offer->is_in_stock() == false) {continue;}
		}
			 
		// пропускаем вариации на предзаказ
		$skip_backorders_products = yfym_optionGET('yfym_skip_backorders_products');
		...
		$no_group_id_arr = unserialize(yfym_optionGET('yfym_no_group_id_arr'));
		if (empty($no_group_id_arr)) {
			...
			if (in_array($CurCategoryId, $no_group_id_arr)) {
			 ...
			 $add_in_name_arr = unserialize(yfym_optionGET('yfym_add_in_name_arr'));
			 $attributes = $product->get_attributes(); // получили все атрибуты товара
			 $param_at_name = '';		

			 $separator_type = yfym_optionGET('yfym_separator_type');			 
			 ...
			 foreach ($attributes as $param) {					
				...
				if ($param_at_name == '') {
					yfym_error_log('Пропускаем товар т.к. нет вариаций для уникальзации заголовков. Файл: functions.php; Строка: '.__LINE__, 0);	
		...
		$result_yml .= '<offer '.$gi.' id="'.$offer_id.'" available="'.$available.'">'.PHP_EOL;		
		...
		$yfym_enable_auto_discounts = yfym_optionGET('yfym_enable_auto_discounts');
		...
		$result_url = apply_filters('yfym_url_filter', $result_url, $product);
		 
		$result_yml .= "<url>".$result_url."</url>".PHP_EOL;
		 
		$yfym_price_from = yfym_optionGET('yfym_price_from');
		...
		// старая цена
		$yfym_oldprice = yfym_optionGET('yfym_oldprice');
		...
		// штрихкод			 
		$yfym_barcode = yfym_optionGET('yfym_barcode');
		...
		$expiry = yfym_optionGET('yfym_expiry');
		...
		$downloadable = yfym_optionGET('yfym_downloadable');
		...
		$country_of_origin = yfym_optionGET('yfym_country_of_origin');
		...
		$sales_notes_cat = yfym_optionGET('yfym_sales_notes_cat');
		if (!empty($sales_notes_cat) && $sales_notes_cat !== 'off') {	
			...
			if (!empty($sales_notes_yml)) {	
				...
				$sales_notes = yfym_optionGET('yfym_sales_notes');
		...
		// гарантия
		$manufacturer_warranty = yfym_optionGET('yfym_manufacturer_warranty');
		...
		$vendor = yfym_optionGET('yfym_vendor');
		if ($vendor !== 'none') {
		...
		$model = yfym_optionGET('yfym_model');
		if ($model !== 'none') {
		...
		// if ($offer_type =='') { 
		$yfym_vendorcode = yfym_optionGET('yfym_vendorcode');
	...
	yfym_error_log('Все вариации выгрузили. Файл: functions.php; Строка: '.__LINE__, 0);	

	return $result_yml; // все вариации выгрузили	
 ...
 if ($skip_backorders_products == 'on') {
	if ($product->get_manage_stock() == true) { // включено управление запасом			  
		if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) {return $result_yml; /*continue;*/}
	} else {
		if ($product->get_stock_status() !== 'instock') {return $result_yml; /*continue;*/}
	}
 }   
 ...
 $result_url = get_permalink($postId); // урл товара
 $result_url = apply_filters('yfym_url_filter', $result_url, $product);
 $result_yml .= "<url>".$result_url."</url>".PHP_EOL;
